...
export function buildBackgroundCanvas(game: GameType): void {
  game.state.map[game.state.currentRoom.y][game.state.currentRoom.x].layout.forEach((row, i) => {
    row.forEach((tile, j) => {
      if (tile.bg) {
        ...
        game.bgCtx.drawImage(
          game.envSpriteSheet,
          tile.bg[0] * 16,
          tile.bg[1] * 16,
          16,
          16,
          tileWidth * (j + 1) - tileWidth,
          tileHeight * (i + 1) - tileHeight,
          tileWidth,
          tileHeight
        );
      }
...
export function renderBackgroundCanvas(game: GameType) {
  game.ctx.drawImage(
    game.bgCanvas,
