    ...
    constructor(game, iX, iY, layout) {
        ...
        layout.forEach((row, i) => {
            row.forEach((tile, j, arr) => {
                if (!tile.entity)
                    return;
                arr[j].mounted = new tile.entity(this.game, this.roomWidth * iX + (this.tileWidth * (j + 1) - this.tileWidth) + this.tileWidth / 2, this.roomHeight * iY +
                    (this.tileHeight * (i + 1) - this.tileHeight) +
                    this.tileHeight / 2, tile.type);
            });
        });
    }
    mount() {
